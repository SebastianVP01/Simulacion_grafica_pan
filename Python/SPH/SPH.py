...
Hydrodynamics (SPH). Uses the super-simple approch of Matthias MÃ¼ller:
https://matthias-research.github.io/pages/publications/sca03.pdf

Simulates the Navier-Stokes Momentum in Lagrangian Form

    Ï Du/Dt = âˆ’ âˆ‡p + Î¼ âˆ‡Â²u + g

u : velocity
Ï : density
p : pressure
Î¼ : dynamics viscosity
g : gravity

Du/Dt : Lagrangian temporal derivative (=Material Derivative)
âˆ‡p    : Pressure Gradient
âˆ‡Â²u   : Velocity Laplacian (=collection of second derivatives)
...
uáµ¢ : The velocity of the i-th smoothed particle
Páµ¢ : The pressure forces acting on the i-th smoothed particle
Váµ¢ : The viscosity forces acting on the i-th smoothed particle
...
Let xáµ¢ be the 2D position of each smoothed particle.
...
1. Compute the rhs for each particle Fáµ¢
    ...
    1.2 Compute the density at each particle's position
        ...
        Ïáµ¢ = (315 M) / (64 Ï€ Lâ¹) âˆ‘â±¼ (LÂ² âˆ’ dáµ¢â±¼Â²)Â³
    
    1.3 Compute the pressure at each particle's position (Îº is the isentropic
        exponent, Ïâ‚€ is a base density)

        páµ¢ = Îº * (Ï âˆ’ Ïâ‚€)

    1.4 Compute the pressure force of each particle

        Páµ¢ = (âˆ’ (45 M) / (Ï€ Lâ¶)) âˆ‘â±¼ âˆ’ (xâ±¼ âˆ’ xáµ¢) / (dáµ¢â±¼) (pâ±¼ + páµ¢) / (2 Ïâ±¼) (L âˆ’ dáµ¢â±¼)Â²
    
    1.5 Compute the viscosity force of each particle

        Váµ¢ = (45 Î¼ M) / (Ï€ Lâ¶) âˆ‘â±¼ (uâ±¼ âˆ’ uáµ¢) / (Ïâ±¼) (L âˆ’ dáµ¢â±¼)
    
    1.6 Add up the RHS

        Fáµ¢ = Páµ¢ + Váµ¢ + G

2. Integrate the Ordinary Differential Equation  "Ï Duáµ¢/Dt = Fáµ¢" with a
   Î”t timestep

    2.1 Update the particles' velocities

        uáµ¢ â† uáµ¢ + Î”t Fáµ¢ / Ïáµ¢
